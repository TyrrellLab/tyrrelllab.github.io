...
const gl = cv && cv.getContext('webgl2', {alpha:true, antialias:false, premultipliedAlpha:true, powerPreference:'high-performance'});
...
if(!gl){
  /* No WebGL2 — hardware acceleration switched off, a blocklisted driver, or
     simply too many live contexts on the machine.  The metal is decoration;
     the thing underneath it is a real navigation link, so leave the host's
     markup alone and drop only the canvas.  The .liquid-plate pill and the
     label carry the button on their own. */
  if(cv && cv.parentNode) cv.parentNode.removeChild(cv);
  host.dataset.liquidMetalFallback = '';
} else {
...
/* The shader only has to run while its button is on screen. Parking it while
   the host is scrolled away costs one observer and saves a full five-pass
   render per frame; the loop itself keeps ticking so the clock stays honest
   and the rim is already mid-travel when the button comes back. */
let onScreen = true;
if(window.IntersectionObserver){
  new IntersectionObserver(es => { onScreen = es[es.length-1].isIntersecting; },
...
/* Pointer and keyboard reactions are off. The metal still breathes at rest,
   but hovering, pressing and focusing no longer light it, drag a well toward
   the cursor, or throw a ripple. Flip REACT to true to restore all of it.
   Keyboard focus is unaffected visually: the focus ring is a plain CSS
   outline on .liquid-button, not part of this shader. */
const REACT = false;
